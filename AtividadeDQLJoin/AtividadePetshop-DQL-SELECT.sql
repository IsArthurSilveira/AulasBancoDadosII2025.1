    ...
    f.email "Email Fornecedor", ifnull(tel.numero, "Não Informado")"Telefone Fornecedor"
		from produtos pd
			...
			left join telefone tel on tel.Fornecedor_cpf_cnpj = f.cpf_cnpj
					order by f.nome;
/*Relatório 12 - Lista dos Produtos mais vendidos, informando a quantidade (total) de vezes que cada produto participou em vendas e o total de valor apurado com a venda do
produto, trazendo as colunas (Nome Produto, Quantidade (Total) Vendas, Valor Total Recebido pela Venda do Produto), ordenado por quantidade de vezes que o produto participou
 em vendas;*/
 select pd.nome "Nome do Produto", count(isvp.Produto_idProduto) "Quantidade (Total) Vendas",
		sum(isvp.valor) "Valor Total Recebido pela Venda do Produto"
	from produtos pd
		join itensvendaprod isvp on pd.idProduto = isvp.Produto_idProduto
			group by pd.nome
				order by count(isvp.Produto_idProduto) desc;